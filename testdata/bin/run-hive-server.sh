...
HADOOP_CLIENT_OPTS="-Xmx${HIVE_METASTORE_MEM_LIMIT_MB}m -Dhive.log.file=hive-metastore.log" \
...
if [ ${ONLY_METASTORE} -eq 0 ]; then
  if [[ $IMPALA_MINICLUSTER_PROFILE == 2 ]]; then
    HADOOP_HEAPSIZE="$HIVE_SERVER_MEM_LIMIT_MB" hive \
      --service hiveserver2 > ${LOGDIR}/hive-server2.out 2>&1 &
  elif [[ $IMPALA_MINICLUSTER_PROFILE == 3 ]]; then
    HADOOP_CLIENT_OPTS="-Xmx${HIVE_SERVER_MEM_LIMIT_MB}m -Dhive.log.file=hive-server2.log" \
