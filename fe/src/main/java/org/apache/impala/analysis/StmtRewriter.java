...
public class StmtRewriter {
  ...
  private static void rewriteSelectStatement(SelectStmt stmt, Analyzer analyzer)
    ...
    if (analyzer.selectBlockContainsPercentile()) rewritePercentileSelectStmt(stmt, analyzer);
  ...
  private static void rewriteWhereClauseSubqueries(SelectStmt stmt, Analyzer analyzer)
    ...
    for (Expr expr: exprsWithSubqueries) {
      if (mergeExpr(stmt, expr, analyzer)) {
  ...
  private static boolean mergeExpr(SelectStmt stmt, Expr expr,
    ...
    // expr = rewriteExpr(expr, analyzer);
    rewriteQueryStatement(inlineView.queryStmt_, inlineView.queryStmt_.getAnalyzer());
    inlineView.reset();
  ...
  private static void rewritePercentileSelectStmt(final SelectStmt stmt,
    ...
    //if (analyzer.getIsinline view() && analyzer.get)
    ...
    // To gather directing slotRefs, we need to first replace 1) aliases 2) ordinals and
    ...
    // between slotRefs.
    // The slotRefs in the select list refer to the from clause while the slotRefs in
    // resultExprs_ doesn't. For consistency we need to substitute using start expanded
    // select list instead of resultExprs_.
    try {
      if (stmt.hasGroupByClause()) {
      ...
      public Expr apply(Expr expr) {
        ...
        if (expr instanceof SlotRef) {
          ...
          if (id == null) {
            ...
            inlineViewSelectList.add(new SelectListItem(expr.clone().reset(),
                redirectAliasPrefix + id));
          }
          return new SlotRef(Lists.newArrayList(PERCENTILE_VIEW_NAME,
              redirectAliasPrefix+ id));
        }
    ...
    InlineViewRef inlineViewRef =
        new InlineViewRef(PERCENTILE_VIEW_NAME, inlineViewStmt, (TableSampleClause) null);
  ...
  private static Expr rewritePercentileExpr(PercentileAggExpr e,
      ArrayList<Expr> groupingExprs, ArrayList<SelectListItem> inlineViewSelectList,
    ...
    String inlineViewColName = "_percentile_row_number_diff_" + inlineViewSelectList.size();
