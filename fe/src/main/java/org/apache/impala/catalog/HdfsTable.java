...
public class HdfsTable extends Table implements FeFsTable {
  ...
  public TGetPartialCatalogObjectResponse getPartialInfo(
    ...
    if (req.table_info_selector.want_partition_list) {
      Preconditions.checkArgument(req.table_info_selector.partition_ids == null,
          "should not specify partition IDs when fetching partition list");
      partIds = partitionMap_.keySet();
    } else {
      partIds = req.table_info_selector.partition_ids;
    }
    ...
    if (partIds != null) {
      ...
      for (long partId : partIds) {
        ...
        // We only need to send back the name if they're doing the initial listing.
        // Otherwise they already know the names.
        if (req.table_info_selector.want_partition_list) {
