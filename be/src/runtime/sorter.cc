...
Status Sorter::Run::PrepareRead(bool* pinned) {
  ...
  int num_to_pin = (!fixed_len_pages_.empty() ? 1 : 0) + (HasVarLenPages() ? 1 : 0);
...
Status
Sorter::Run::AddIntermediateBatch(RowBatch* batch, int start_index, int* num_processed) {
  DCHECK(!initial_run_);
  if (has_var_len_slots_) {
...
bool Sorter::Run::HasVarLenPages() const {
...
Status Sorter::TupleSorter::Codegen(llvm::Function* compare_fn, RuntimeState* state) {
  ...
  llvm::Function* replaced = codegen->ReplaceCallSitesRecursive(
      sort_helper_fn, codegen->GetFunction(IRFunction::COMPARE_INTERPRETED, false),
      compare_fn);
