...
llvm::Function* LlvmCodeGen::ReplaceCallSitesRecursive(llvm::Function* caller,
  ...
  struct FunctionInfo {
    ...
    // discovered.
    uint64_t dfs_index;
    // The smallest index of any node known to be reachable from this function.
    uint64_t low_link;
  ...
  while (!dfs_stack.empty()) {
    ...
    if (dfs_stack.top().iter.atEnd()) {
      if (fn_info->second.dfs_index == fn_info->second.low_link) {
        ...
        do {
          unassigned_top = scc_unknown_stack.top();
          ...
          if (fn_info->second.old_fn_reachable) {
            ...
            llvm::Function* cloned_fn =
                llvm::CloneFunction(unassigned_top, old_new_fn_mapping);
            cloned_fn->copyAttributesFrom(unassigned_top);
            cloned_scc_members.emplace_back(cloned_fn);
            cloned_fn_map.insert({unassigned_top, llvm::WeakTrackingVH(cloned_fn)});
          }
