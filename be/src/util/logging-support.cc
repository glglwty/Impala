...
Status GetJavaLogLevel(const TGetJavaLogLevelParams& params, string* result) {
  RETURN_IF_ERROR(JniCall(get_log_level_method)
      .on_class(log4j_logger_class_).with_thrift_arg(params).Call(result));
  return Status::OK();
}

Status ResetJavaLogLevels() {
  RETURN_IF_ERROR(JniCall(reset_log_levels_method).on_class(log4j_logger_class_).Call());
  return Status::OK();
}
...
void SetJavaLogLevelCallback(const Webserver::ArgumentMap& args, Document* document) {
  ...
  Status status = JniCall(set_log_level_method).on_class(log4j_logger_class_)
